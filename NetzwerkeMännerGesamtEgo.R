### Gesamtnetzwerke und Egonetzwerke erstellen
...
Frauen<-V(m)[sex=="w"]
Frauen
V(m)[Frauen]$color="maroon1"

Men<-V(m)[sex=="m"]
Men
V(m)[Men]$color="skyblue1"

fav_sex <- plot(m, layout = layout_with_kk, main="Geschlechtsverteilung")

# Knotenform nach Typ festlegen
Spieler<-V(m)[person=="Player"]
Spieler
V(m)[Spieler]$shape="square"

Bekannte<-V(m)[person=="Friend"]
Bekannte
V(m) [Bekannte]$shape="circle"

m_freunde <- plot(m, layout = layout_with_kk, main="Freunde und Bekannte")
